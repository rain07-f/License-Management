...
@section('content')
    <div class="container-fluid p-0">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h4 class="mb-1 fw-bold text-secondary">{{ $application->name }} Versions</h4>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-transparent p-0 mb-0">
                        <li class="breadcrumb-item"><a href="{{ route('admin.applications.index') }}"
                                class="text-secondary text-decoration-none">Applications</a></li>
                        ...
                        <tbody>
                            @forelse ($application->versions as $version)
                                <tr>
                                    <td class="px-4 py-3">
                                        <span
                                            class="badge bg-primary-subtle text-primary fw-bold px-3 fs-6">{{ $version->version }}</span>
                                    </td>
                                    <td class="py-3">
                                        <small class="text-secondary text-wrap"
                                            style="max-width: 300px; display: inline-block;">{{ Str::limit($version->release_notes, 100) }}</small>
                                    </td>
                                    <td class="py-3 text-center">
                                        @if($version->min_php_version)
                                            <span class="badge bg-light text-dark mb-1">PHP:
                                                {{ $version->min_php_version }}+</span><br>
                                        @endif
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="text-center py-5">
                                        <div class="text-muted mb-2"><i data-lucide="upload-cloud"
                                                class="icon-lg opacity-50"></i></div>
    ...
    <div class="modal fade" id="uploadVersionModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form action="{{ route('admin.application-versions.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="modal-body p-4 pt-4">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3 mb-md-0">
                            ...
                            <input type="text" name="min_php_version" class="form-control rounded-pill px-3"
                                placeholder="e.g. 8.1" />
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-medium">Min WP Version (Optional)</label>
                            <input type="text" name="min_wp_version" class="form-control rounded-pill px-3"
                                placeholder="e.g. 6.4" />
                        </div>
                    </div>
                </div>
